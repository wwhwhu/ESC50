...
from model.model import CNN_k2c2_model
...
# get CNN model
model = CNN_k2c2_model(x_train.shape[1:], class_num=int(y_train.shape[1:][0]))
model.summary()
...
# 创建回调函数实例
log_metrics_cb = LogMetrics('CNN_models\\metrics.csv')
...
class MixupGenerator():
    ...
    # 首先，从训练集中选取出两组输入和标签，分别为 x1、x2、y1、y2。然后，从 Beta 分布中随机采样出一个混合比例 l。
...
model.load_weights("CNN_models\\best.hdf5")
...
plt.savefig("CNN_models\\acc.png")
plt.show()
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig("CNN_models\\loss.png")
plt.show()