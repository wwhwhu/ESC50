...
def quant1(converter):
    ...
    with open('compression/CRNN/quant1.tflite', 'wb') as f:
...
def quant2(converter):
    ...
    # 保存 TensorFlow Lite 模型
    with open('compression/CRNN/quant2.0.tflite', 'wb') as f:
...
def quant2_x(converter):
    ...
    # 保存 TensorFlow Lite 模型
    with open('compression/CRNN/quant2.1.tflite', 'wb') as f:
...
def quant3(converter):
    ...
    # 保存 TensorFlow Lite 模型
    with open('compression/CRNN/quant3.tflite', 'wb') as f:
...
h5_model = tf.keras.models.load_model("CRNN_models/best.hdf5")
h5_model.summary()
...
# 保存 TensorFlow Lite 模型
with open('compression/CRNN/original.tflite', 'wb') as f:
...
def prune1(model, train_data, train_labels, test_data, test_labels):
    ...
    x1 = get_gzipped_model_size("CRNN_models/best.hdf5", "CRNN_models/best_h5.zip")
    x2 = get_gzipped_model_size("compression/CRNN/original.tflite", "compression/CRNN/original_tf.zip")
    batch_size = 16
    epochs = 20
    ...
    tf.keras.models.save_model(model_for_export, "compression/CRNN/prune1_ex.hdf5", include_optimizer=False)
    ...
    with open("compression/CRNN/prune1_ex.tflite", "wb") as f:
        f.write(tflite_model2)
    x3 = get_gzipped_model_size("compression/CRNN/prune1_ex.hdf5", "compression/CRNN/prune1_ex_h5.zip")
    x4 = get_gzipped_model_size("compression/CRNN/prune1_ex.tflite", "compression/CRNN/prune1_ex_tf.zip")
    print("Size of gzipped baseline Keras model: %.2f bytes" % x1)  # 2079.63K
    print("Size of gzipped baseline TFlite model: %.2f bytes" % x2)  # 527.82K
    print("Size of gzipped pruned Keras model: %.2f bytes" % x3)  # 325.86K
    print("Size of gzipped pruned TFlite model: %.2f bytes" % x4)  # 301.66K
...
def prune2(model, train_data, train_labels, test_data, test_labels):
    ...
    # 定义剪枝方法Conv2D与batch剪枝
    def apply_pruning_to_dense(layer):
        if isinstance(layer, tf.keras.layers.Dense) or isinstance(layer, tf.keras.layers.LSTM):
            print("find prune layer:", layer.name)
            return sparsity.keras.prune_low_magnitude(layer, **pruning_params)
        return layer
    ...
    model_for_export = sparsity.keras.strip_pruning(model_for_pruning)
    model_for_export.summary()
    tf.keras.models.save_model(model_for_export, "compression/CRNN/prune2_ex.hdf5", include_optimizer=False)
    converter2 = tf.lite.TFLiteConverter.from_keras_model(model_for_export)
    converter2.experimental_enable_resource_variables = True
    converter2.optimizations = [tf.lite.Optimize.EXPERIMENTAL_SPARSITY]
    converter2.target_spec.supported_ops = [tf.lite.OpsSet.TFLITE_BUILTINS, tf.lite.OpsSet.SELECT_TF_OPS]
    tflite_model2 = converter2.convert()
    with open("compression/CRNN/prune2_ex.tflite", "wb") as f:
        f.write(tflite_model2)
    x3 = get_gzipped_model_size("compression/CRNN/prune2_ex.hdf5", "compression/CRNN/prune2_ex_h5.zip")
    x4 = get_gzipped_model_size("compression/CRNN/prune2_ex.tflite", "compression/CRNN/prune2_ex_tf.zip")
    print("Size of gzipped baseline Keras model: %.2f bytes" % x1)  # 2079.63K
    print("Size of gzipped baseline TFlite model: %.2f bytes" % x2)  # 527.82K
    print("Size of gzipped pruned Keras model: %.2f bytes" % x3)  # 377.68K
    print("Size of gzipped pruned TFlite model: %.2f bytes" % x4)  # 359.45K
